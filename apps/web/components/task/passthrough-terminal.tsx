...
function useTouchScrollFallback(
  ...
  React.useEffect(() => {
    ...
    // Read xterm from the ref inside each handler so we tolerate the
    // underlying instance being recreated mid-effect (e.g. WebGL→canvas
    // renderer swap on resize). React's ref object is stable so the effect
    // doesn't re-run when .current changes; capturing once would freeze a
    // stale Terminal reference.
    const getTerm = () => xtermRef.current;

    // Take ownership of touch gestures so the page doesn't pull-to-refresh.
    // Track whether the property was previously inline-set so cleanup can
    // remove it rather than leaking an empty string in place of "no inline
    // style at all" — those behave the same for touch-action today but the
    // distinction matters for cascade correctness.
    const hadInlineTouchAction = el.style.touchAction !== "";
    const prevTouchAction = el.style.touchAction;
    el.style.touchAction = "none";
    const hadInlineOverscroll = el.style.overscrollBehavior !== "";
    const prevOverscroll = el.style.overscrollBehavior;
    el.style.overscrollBehavior = "contain";

    // xterm's cell height: prefer the public option (stable API) with the
    // measured element height as a refinement; fall back to 16 only as a
    // last resort. The internal _core._renderService path was previously
    // used here but it's not API-stable across xterm major versions.
    const rowHeightPx = () => {
      const t = getTerm();
      if (!t) return 16;
      if (t.rows > 0 && el.clientHeight > 0) {
        const measured = el.clientHeight / t.rows;
    ...
    let pendingRows = 0;
    // ~50ms of (timestamp, pixel-delta) pairs — short enough that a
    // direction reversal at the end of the gesture is reflected in the
    // release velocity rather than averaged out against earlier motion.
    ...
    // Decay velocity to zero. 0.94 per 16ms gives a feel close to iOS
    // scroll on the Files panel.
    ...
    const onMove = (e: TouchEvent) => {
      const t = getTerm();
      if (!t || lastY === null || e.touches.length !== 1) return;
      e.preventDefault();
      ...
      while (samples.length > 0 && now - samples[0].t > 50) samples.shift();
      // Reset accumulated fractional rows when direction reverses so the
      // new direction's first row fires immediately instead of paying off
      // residual carry from the previous direction.
      if ((dy < 0 && pendingRows > 0) || (dy > 0 && pendingRows < 0)) {
        pendingRows = 0;
      }
    ...
    const onEnd = () => {
      ...
      // Don't fire inertia for tiny / slow releases — would drift after a tap.
    ...
    return () => {
      ...
      if (hadInlineTouchAction) el.style.touchAction = prevTouchAction;
      else el.style.removeProperty("touch-action");
      if (hadInlineOverscroll) el.style.overscrollBehavior = prevOverscroll;
