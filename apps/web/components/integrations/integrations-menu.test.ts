...
const setActiveWorkflowMock = vi.hoisted(() => vi.fn());
...
vi.mock("@/components/state-provider", () => ({
  useAppStore: (
    selector: (state: {
      ...
      setActiveWorkflow: typeof setActiveWorkflowMock;
    }) => unknown,
  ) =>
    selector({
      workspaces: { activeId: activeWorkspaceRef.id },
      setActiveWorkspace: setActiveWorkspaceMock,
      setActiveWorkflow: setActiveWorkflowMock,
    }),
}));
